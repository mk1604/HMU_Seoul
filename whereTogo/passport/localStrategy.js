...
const JWTVerify = async (jwtPayload, done) => {
    try {
        ...
        let user = User.findOne({where : jwtPayload.user_id})
        // 유효한 유저라면
        if (user) {
...
const passportConfig = { usernameField: 'user_id', passwordField : 'password' };

passport.use(
    'signIn',
    new LocalStrategy(passportConfig, async (user_id, password, done) => {
